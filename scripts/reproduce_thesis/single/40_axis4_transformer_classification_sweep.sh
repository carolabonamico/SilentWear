...
#
# 40 axis4 transformer classification sweep
# ==============================
#
...
start_log "40_axis4_transformer_classification_sweep"

SUBJECTS_RUN="$SUBJECTS"
BASE=config_thesis/thesis_base_sentences_w2000_rest.yaml
MODEL=config_thesis/models_configs/sentences_stft_ctc_transformer_classification_sweep.yaml
ROOT="$ARTIFACTS_BASE/40_axis4_decoder_sweep/transformer_classification"

for EXP in $EXPERIMENTS; do
    [ "$SKIP_TRAIN" != "1" ] && train "$BASE" "$MODEL" "$DATA_SENTENCES" "$ROOT" "$EXP" 2.0
    beam_sweep  "$ROOT" "$EXP"
    beam_tables "$ROOT" "$EXP" speechnet_transformer w2000ms
done

done_msg "40_axis4_transformer_classification_sweep" "$ROOT" "Tables 4.7 and 6.2"
