...
class Global_Windower_and_Feature_Extractor:
    ...
    def __init__(
        ...
        """Initialize the windower / feature extractor for a single subject.

        Args:
            subject_config: parsed configuration for the current subject. When
                ``manual_feature_extraction`` is True, handcrafted features are
                returned in addition to the raw windows.
            conditions: optional set of condition folder names (e.g. {"silent"})
                to restrict processing to. When None, all conditions found under
                the subject's processed folder are windowed.
        """
        self.data_directory = subject_config.data_directory
        self.sub_id = subject_config.subject_id
        self.raw_and_filtered_dirname = subject_config.raw_and_filtered_dirname
        self.wins_and_features_dirname = subject_config.wins_and_features_dirname
        ...
        self.conditions = set(conditions) if conditions else None
        self.dire_wins_feats_silent: Optional[Path] = None
        self.dire_wins_feats_vocalized: Optional[Path] = None
    ...
    def find_all_processed_h5(self) -> list:
        ...
        When a condition filter was provided, only recordings whose parent folder
        matches one of the requested conditions are returned.
        """
        h5_files = sorted(
            (self.data_directory / self.raw_and_filtered_dirname / self.sub_id).rglob("*.h5")
        )
        if self.conditions is not None:
            h5_files = [f for f in h5_files if f.parent.name in self.conditions]
        return h5_files

    def create_saving_directory(self) -> None:
        """Create the output directories for the extracted windows/features."""
        self.data_dire_wins_and_feats = self.data_directory / self.wins_and_features_dirname
        win_subdir = f"{WINDOW_DIR_PREFIX}{int(self.win_size_sec * 1000)}"
    ...
    def main(self):
        ...
        for curr_h5_file in h5_files:
            ...
            df_save_path = None
            if self.save_data:
                if curr_h5_file.parent.name == SILENT_DIRNAME:
                    print("Silent recording")
                    df_save_path = self.dire_wins_feats_silent / curr_h5_file.name
                elif curr_h5_file.parent.name == VOCALIZED_DIRNAME:
